...
{//Clase para crear un objeto validador para validar inputs

    public class Validador
    {
        // Validacion numerica 
        public static bool validarNum(String str, TextBox txtBox) {
            ...
            try
            {
                ...
                if (!(str.All(char.IsDigit)) && !(flg))
                {
                    if (decimal.Parse(str) < 0)
                    {
                        throw new Exception("Solo se pueden digitar numeros POSITIVOS");
                    }
                    else
                    {
                        throw new Exception("Solo se pueden digitar numeros");
                    }

                }


                if (str.Trim() != string.Empty && flg)
                {
                    // se regresa el true 
                    return true;
                }
                else
                {
                    ...
                    throw new Exception("Ingrese datos correctos");
                }
            ...
            catch (Exception e)
            {
                ...
                MessageBox.Show("Dato erroneo :" + e.Message);
        ...
        // validacion char 
        public static bool validarChar(string str, TextBox txt)
        {
            try {
                ...
                if (str.Trim() != string.Empty)
                {
                    if (txt.Text.All(char.IsLetter)) {
                        return true;
                    } else if (ValidaEspacio(str.Trim())) {
                    ...
                    if (!(txt.Text.All(char.IsLetter)))
                    {
                        ...
                        throw new Exception("no deje campos en blanco");
        ...
        public static bool ValidaEspacio(string frase)
        
        {

            string aux = Regex.Replace(frase," ", "");
            if (aux.All(char.IsLetter))
            {
                return true; // se valida correctamente
            }
            return false;
